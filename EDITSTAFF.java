...
public class EDITSTAFF extends JFrame {
	...
	String header[]= new String[]{"Staff Name","salary","Tel","Age"};
	...
	public void displayDetails() {
		...
		for(int i=0; i<stafflist.size();i++){
			Object[] obj={stafflist.get(i).name,stafflist.get(i).sid,stafflist.get(i).tel,stafflist.get(i).age};
			//Object[] obj={AddUser.people.get(i),AddUser.gender.get(i),AddUser.phoneAr.get(i)};
			System.out.println(stafflist.get(i).name);
	...
	private void initialize() {
		...
		JLabel lblStudentId = new JLabel("Salary");
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				stafflist.get(row).name=name.getText();
				stafflist.get(row).sid=sid.getText();
				stafflist.get(row).tel=section.getText();
				stafflist.get(row).age=enrollment.getText();
				displayDetails() ;
			}
		});
	...
	private void saveToFile() {
		try{
			FileWriter out = new FileWriter ("STAFFNEW.csv", true);
				
			
			//for(int i=0; i< stafflist.size(); i++) {
			out.write(stafflist.get(stafflist.size()-1).name.toString()+","+stafflist.get(stafflist.size()-1).sid.toString()+","+"(+66)"+stafflist.get(stafflist.size()-1).tel.toString()+","
				+stafflist.get(stafflist.size()-1).age.toString()+"\n");
			//}
